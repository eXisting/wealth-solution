...
const CalculatorWithSnapsCombination = () => {
  ...
  useEffect(() => {
    updateDecadeTotalSavings(decadeOneMonthlyContribution, decadeOneAge, startingSavings, updateFirstDecadeTotalSavings);
  }, [decadeOneMonthlyContribution, decadeOneAge, startingSavings]);

  useEffect(() => {
    updateDecadeTotalSavings(decadeTwoMonthlyContribution, decadeTwoAge, decadeOneTotalSavings, updateSecondDecadeTotalSavings);
  }, [decadeTwoMonthlyContribution, decadeTwoAge, decadeOneTotalSavings]);

  useEffect(() => {
    
    updateDecadeTotalSavings(decadeThreeMonthlyContribution, decadeThreeAge, decadeTwoTotalSavings, updateThirdDecadeTotalSavings);
  }, [decadeThreeMonthlyContribution, decadeThreeAge, decadeTwoTotalSavings]);
  ...
  const ageIntervalChanged = (value, stageNumber) => {
    if (stageNumber === 1) {
      ...
      dispatch(updateFirstDecadeAge(value));
    } else if (stageNumber === 2) {
      dispatch(updateSecondDecadeEnabled(true));
      dispatch(updateSecondDecadeAge(value));
    } else if (stageNumber === 3) {
      dispatch(updateThirdDecadeEnabled(true));
      dispatch(updateThirdDecadeAge(value));
    }
  };
  ...
  return (
    <Container>
      ...
      <Section ignore style={{color:"grey"}}>
        <HorizontalStack>
          <span style={{ fontSize: '2.2vh', textAlign:"center"}}>Your age</span>
          <NumbericSnaps
              min={0} 
              max={50}
              initialValue={Number(startingAge)+Number(decadeThreeAge)+Number(decadeOneAge)+Number(decadeTwoAge)}
              inputFieldHeight={"3vh"}
              inputFieldWidth={"7vh"}
              ...
              readOnly={true}
            />
          <span style={{ fontSize: '2.2vh', textAlign:"center", marginLeft:"1vh"}}>Years saved</span>
          <NumbericSnaps 
            min={0} 
            max={99}
            initialValue={Number(decadeThreeAge)+Number(decadeOneAge)+Number(decadeTwoAge)}
            custom={true}
            inputFieldHeight={"3vh"}
            inputFieldWidth={"7vh"}
            disableControls={true}
      ...
      <Section ignore width="100%" style={{border:"0.1vh solid #000000", height:"0px"}} />
      <Section ignore maxHeight={"50%"}>
        <HorizontalStack>
          <span style={{fontSize:"2.5vh"}}>
            Your 1st life savings stage 
          </span>
          <Switch 
            checked={decadeOneEnabled}
            onChange={(value, mause) => changeStageEnabled(value, 1)} 
            style={{ marginLeft:"2vh", background: decadeOneEnabled ? '#0BDA51' : 'gray' }}
          />
        </HorizontalStack>
        <SavingsSection
          age={decadeOneAge}
      ...
      <Section ignore align="top" maxHeight={"50%"}>
        <HorizontalStack>
          <span style={{fontSize:"2.5vh"}}>
            Your 2nd life savings stage 
          </span>
          <Switch 
            checked={decadeTwoEnabled}
            onChange={(value, mause) => changeStageEnabled(value, 2)} 
            style={{ marginLeft:"2vh", background: decadeTwoEnabled ? '#0BDA51' : 'gray' }}
          />
        </HorizontalStack>
        <SavingsSection
          age={decadeTwoAge}
      ...
      <Section ignore align="top" maxHeight={"50%"}>
        <HorizontalStack>
          <span style={{fontSize:"2.5vh"}}>
            Your 3rd life savings stage 
          </span>
          <Switch 
            checked={decadeThreeEnabled}
            onChange={(value, mause) => changeStageEnabled(value, 3)} 
            style={{ marginLeft:"2vh", background: decadeThreeEnabled ? '#0BDA51' : 'gray' }}
          />
        </HorizontalStack>
        <SavingsSection
          age={decadeThreeAge}
      ...
      <Section ignore width="100%" style={{border:"0.1vh solid #000000", height:"0px", marginTop:"2vh"}} />
      <Section justify={"top"} style={{marginTop:"2vh", marginBottom:"4vh"}}>
